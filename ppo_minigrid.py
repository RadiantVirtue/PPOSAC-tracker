# Adapted from CleanRL's PPO implementation for Minigrid environments
# Original: https://docs.cleanrl.dev/rl-algorithms/ppo/#ppopy
import os
import random
import time
...
def make_env(env_id, idx, capture_video, run_name):
    def thunk():
        if capture_video and idx == 0:
            ...
            env = gym.make(env_id)
        env = ImgObsWrapper(env)  # Use only the image observation (7x7x3 by default)
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Input: (batch, H, W, C) -> need to permute to (batch, C, H, W)
        ...
        with torch.inference_mode():
            ...
            cnn_output_dim = self.network(sample).shape[1]
