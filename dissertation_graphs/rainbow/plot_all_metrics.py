...
Per-metric timeseries for Rainbow.  One PNG per metric saved to rainbow/output/metrics/.
...
Metrics produced:
  ...
  Weight-delta (Rainbow-only):
           cos(G_uniform/IS/reward, delta_theta) for success and failure groups,
           cos(G_uniform, G_IS) for success and failure groups
...
from shared.graphing import (
    ...
    _C_UNIFORM, _C_IS, _C_REWARD,
    C_DELTA_UNIFORM_S, C_DELTA_IS_S, C_DELTA_REWARD_S,
    C_DELTA_UNIFORM_F, C_DELTA_IS_F, C_DELTA_REWARD_F,
...
# Rainbow-only weight-delta and IS-variant metrics
_RAINBOW_EXTRA_METRICS = [
    # cos(G_variant, delta_theta) — success group
    ("cos_uniform_success_delta", C_DELTA_UNIFORM_S, "cos(G_uniform, delta_theta) success", "cos_uniform_success_delta"),
    ("cos_is_success_delta",      C_DELTA_IS_S,      "cos(G_IS, delta_theta) success",      "cos_is_success_delta"),
    ("cos_reward_success_delta",  C_DELTA_REWARD_S,  "cos(G_reward, delta_theta) success",  "cos_reward_success_delta"),
    # cos(G_variant, delta_theta) — failure group
    ("cos_uniform_failure_delta", C_DELTA_UNIFORM_F, "cos(G_uniform, delta_theta) failure", "cos_uniform_failure_delta"),
    ("cos_is_failure_delta",      C_DELTA_IS_F,      "cos(G_IS, delta_theta) failure",      "cos_is_failure_delta"),
    ("cos_reward_failure_delta",  C_DELTA_REWARD_F,  "cos(G_reward, delta_theta) failure",  "cos_reward_failure_delta"),
    # cos(G_uniform, G_IS) — alignment between weighting schemes
    ("cos_uniform_is_success",    _C_UNIFORM,        "cos(G_uniform, G_IS) success",        "cos_uniform_is_success"),
    ("cos_uniform_is_failure",    _C_UNIFORM,        "cos(G_uniform, G_IS) failure",        "cos_uniform_is_failure"),
]
...
def _plot_scalar(results, step_grid, ach_steps, field, color,
    ...
    ax.set_title(f"Rainbow — {ylabel}  (mean +/- 1 std, {n} seeds)")
    ...
    return _save_fig(fig, out_dir, f"{stem}_rainbow.png", dpi)
...
def plot(rainbow_root: str, out_dir: str, dpi: int = 150) -> list:
    ...
    all_scalar = _SCALAR_METRICS + _RAINBOW_EXTRA_METRICS
    print(f"  Generating {len(all_scalar)} scalar metric graphs...")
    for field, color, ylabel, stem in all_scalar:
...
def main():
    ...
    parser.add_argument("--out_dir",
                        ...
                        help="Output directory (default: rainbow/output/metrics/)")
