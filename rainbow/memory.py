...
Transition_dtype = np.dtype([('timestep', np.int32), ('state', np.uint8, (3, 64, 64)), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.bool_)])
blank_trans = (0, np.zeros((3, 64, 64), dtype=np.uint8), 0, 0.0, False)
...
class ReplayMemory():
  ...
  def append(self, state, action, reward, terminal):
    state = state.mul(255).to(dtype=torch.uint8, device=torch.device('cpu'))  # Discretise full RGB frame to uint8
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    ...
    # all_states shape: (B, history+n, 3, 64, 64) with history=1 → squeeze dim 1
    all_states = transitions['state']
    states = torch.tensor(all_states[:, 0], device=self.device, dtype=torch.float32).div_(255)           # (B, 3, 64, 64)
    next_states = torch.tensor(all_states[:, self.n], device=self.device, dtype=torch.float32).div_(255) # (B, 3, 64, 64)
  ...
  def __next__(self):
    ...
    transitions[blank_mask] = blank_trans
    state = torch.tensor(transitions['state'][-1], dtype=torch.float32, device=self.device).div_(255)  # (3, 64, 64)
    self.current_idx += 1
    return state
